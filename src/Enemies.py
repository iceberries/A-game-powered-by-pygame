...
class Enemy(Image, pygame.sprite.Sprite):
    ...
    def should_runaway(self, target, runaway_distance=300, runaway_duration=30,speed_plus=1):
    ...
    def move_behead_attacker(self, attack_center, player_pos, keep_distance=200, runaway_duration=30, player_runaway_distance=200, player_runaway_duration=20, enemies=None):
        my_center = pygame.Vector2(self.getrect().center)
        ...
        # 玩家过于靠近时逃跑
        runaway_player, player_escape_vec = self.should_runaway(player_pos, player_runaway_distance, player_runaway_duration,2)
        if runaway_player:
            move_vec = player_escape_vec
        else:
            # 使用should_runaway统一逃跑判定（对群体中心）
            runaway, move_vec = self.should_runaway(attack_center, keep_distance, runaway_duration,1)
            if not runaway:
...
class AttackEnemy(Enemy):
    ...
    def play_animation(self):
        if self.is_attacking:
            # 播放攻击动画
            idx = min(self.attack_frame // (self.attack_cooldown // 3), len(self.attack_paths) - 1)
